...
export class Session {
  ...
  getLiveProgress(): { progress: Progress; completed: boolean } {
    const { length } = this.#steps;
    if (length === 0) {
      return { progress: Session.emptyProgress, completed: false };
    }
    const head = this.#steps[0];
    const time = performance.now() - head.timeStamp;
    const speed = computeSpeed(length, time);
    const { duration } = this.settings;
    let progressValue = 0;
    let completed = false;
    if (duration.type === DurationType.Time) {
      progressValue = Math.min(1, time / duration.value);
      completed = time >= duration.value;
    } else {
      progressValue = Math.min(1, length / duration.value);
      completed = length >= duration.value;
    }
    return {
      progress: { time, length, progress: progressValue, speed },
      completed,
    };
  }
